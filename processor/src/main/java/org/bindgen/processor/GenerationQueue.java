 ...
 *
 * As each binding is generated, its properties may need their own binding
 * classes generated, so they get put in this queue.
 */
public class GenerationQueue {

	// Both Eclipse and javac will use the same processor instance for each
	// compilation run (e.g. across all of the rounds), so this should be
	// cumulative
	...
	/**
	 * Enqueue <code>element</code> even if it was written during a previous
	 * compilation run.
	 */
	...
	/**
	 * Enqueue <code>element</code> only if we haven't seen it either this
	 * compilation run or during a previous compilation run.
	 */
	...
	/**
	 * Outputs <code>message</code> to System.out, mostly useful for Debug As
	 * Eclipse/javac debugging.
	 */
	 ...
	 *
	 * We recursively walk into bindings, so first check our in-memory set of
	 * what we've seen so far this compilation run. Eclipse creates a new
	 * processor instance every time the user hits save, so this only has things
	 ...
	 *
	 * If we haven't seen it this compilation run, see if we've already output
	 * awhile ago (e.g. last time they hit save), in which case we can skip it.
	 * If we really needed to do this element again, Eclipse would have deleted
	 * our last output and this check wouldn't find anything. But this does not
	 * work for javac, so we don't do it if told not to. Would be nice to
	 * auto-detect javac.
	 */
	private boolean hasAlreadyBeenWrittenByAPreviousCompilationRun(TypeElement element) {
		...
		try {
			...
			FileObject fo = getFiler().getResource(StandardLocation.SOURCE_OUTPUT, bindingClassName.getPackageName(),
